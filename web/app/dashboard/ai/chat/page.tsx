...
export default function AIChatPage() {
  ...
  useEffect(() => {
    async function loadSessions() {
      const data = await getRecentAIChats(userId);
      setSessions(data);
    }
  ...
  return (
    <div className="grid h-[calc(100vh-130px)] gap-3 lg:grid-cols-[0.3fr_0.7fr]">
      <aside className="rounded-xl border border-[#2a2b2e] bg-[#161719] p-3">
        ...
        {sessions.map((item) => <button key={item.id} className="mb-1 block w-full rounded border border-[#2a2b2e] bg-[#111216] px-2 py-2 text-left text-xs text-[#9ca3af]">{item.title}</button>)}
      </aside>
